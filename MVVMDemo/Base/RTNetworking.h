...
// 项目打包上线都不会打印日志，因此可放心。
#ifdef DEBUG
#define RTAppLog(s, ... ) NSLog( @"[%@ in line %d] ===============>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define RTAppLog(s, ... )
#endif
 ...
 */
typedef void (^RTDownloadProgress)(int64_t bytesRead,
 ...
 */
typedef void (^RTUploadProgress)(int64_t bytesWritten,
                                 int64_t totalBytesWritten);

typedef RTDownloadProgress RTGetProgress;
typedef RTDownloadProgress RTPostProgress;

typedef NS_ENUM(NSUInteger, RTResponseType) {
    kRTResponseTypeJSON = 1, // 默认
    kRTResponseTypeXML  = 2, // XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    RTResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, RTRequestType) {
    kRTRequestTypeJSON = 1, // 默认
    kRTRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, RTNetworkStatus) {
    kRTNetworkStatusUnknown          = -1,//未知网络
    kRTNetworkStatusNotReachable     = 0,//网络无连接
    kRTNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kRTNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionTask RTURLSessionTask;
typedef void(^RTResponseSuccess)(id response);
typedef void(^RTResponseFail)(NSError *error);
...
/**
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的RTURLSessionTask对象，
